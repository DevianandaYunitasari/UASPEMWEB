      ...
      <a href="{{ route('transaksi.show', $data->kode) }}">
        <div class="col-lg-6 mb-4">
          <div class="card o-hidden border-0 shadow h-100">
            <div class="card-body">
              <div class="row no-gutters align-items-center">
                <div class="col font-weight-bold h5" style="margin: 0; color: #000;">
                  {{ $data->jadwal->start }}<i class="fas fa-long-arrow-alt-right mx-2" style="color: #858796;"></i>{{ $data->jadwal->end }}
                </div>
            ...
            <div class="card-body">
              <p style="margin: 0; color:#000;">{{ date('l, d F Y H:i', strtotime($data->fasilitas)) }} WIB</p>
              <p style="margin: 0; color:#000;">{{ $data->jadwal->lapangan->name }} ({{ $data->jadwal->lapangan->kode }})</p>
            </div>
          </div>
      </a>
    </div>
