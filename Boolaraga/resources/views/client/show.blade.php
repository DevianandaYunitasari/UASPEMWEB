  ...
  <div class="col-12" style="margin-top: -15px">
    ...
    <div class="row mt-2">
      ...
      <div class="col-lg-6 mb-4">
        ...
        <div class="card o-hidden border-0 shadow h-100 py-2">
          <div class="card-body">
            <div class="row no-gutters align-items-center">
              <div class="col mr-2">
                <div class="font-weight-bold text-muted text-uppercase mb-1">{{ $data['lapangan'] }}</div>
                <small class="text-muted">{{ $data['lapangan'] }} ({{ $data['kode'] }})</small>
              </div>
        ...
        @else
        <a href="{{ route('cari.antri', Crypt::encrypt($data)) }}">
          <div class="card o-hidden border-0 shadow h-100 py-2">
            <div class="card-body">
              <div class="row no-gutters align-items-center">
                <div class="col mr-2">
                  <div class="font-weight-bold text-gray-800 text-uppercase mb-1">{{ $data['lapangan'] }}</div>
                  <small class="text-muted">{{ $data['lapangan'] }} ({{ $data['kode'] }})</small>
                </div>
                <div class="col-auto text-right">
                  <div class="h5 mb-0 font-weight-bold text-primary">Rp. {{ number_format($data['harga'], 0, ',', '.') }}</div>
                  @if ($data['antri'] < 50) <p class="text-primary" style="margin: 0;"><small>{{ $data['antri'] }} Antrian Tersedia</small></p>
                    @endif
                </div>
              </div>
            </div>
